                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs text-gray-500">
                            Manage instructor profiles, permissions, and account status
                        </p>
                    </div>
                    <Link
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-lg bg-white p-4 border border-gray-200 shadow-xs">
                        ...
                        <div className="text-xs text-gray-600 font-medium shrink-0 whitespace-nowrap bg-gray-50 border border-gray-200 px-3 py-2 rounded-lg">
                            Total Instructors: <strong className="text-gray-900 font-bold ml-1">{instructors?.total || 0}</strong>
                        </div>
                    </div>
                                            ...
                                            <tr key={instructor.id} className="hover:bg-gray-50 transition">
                                                <td className="py-3 px-4">
                                                    <div className="flex items-center space-x-3">
                                                        <div className="h-8 w-8 rounded-md bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-xs shrink-0 border border-blue-200">
                                                            {instructor.name ? instructor.name.charAt(0).toUpperCase() : 'I'}
                                                        </div>
                                                        <div>
                                                            <div className="font-semibold text-gray-900 text-xs">{instructor.name}</div>
                                                            <div className="text-[11px] text-gray-400">ID: #{instructor.id}</div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="py-3 px-4 font-medium text-gray-700">{instructor.email}</td>
                                                <td className="py-3 px-4 font-medium text-gray-600">{instructor.phone || 'N/A'}</td>
                                                <td className="py-3 px-4">
                                                    {getStatusBadge(instructor.status)}
                                                </td>
                                                <td className="py-3 px-4 text-right">
                                                    <Link
                                                        href={route('admin.instructors.show', instructor.id)}
